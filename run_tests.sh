...
function usage {
  echo "Usage: $0 [OPTION]..."
  echo "Run spanner's test suite(s)"
  echo ""
...
function run_pep8 {
  ...
  DJANGO_SETTINGS_MODULE=spanner.test.settings ${command_wrapper} flake8 $included_dirs
}
...
function run_pep8_changed {
    ...
    warn_on_flake8_without_venv
    diff -u --from-file /dev/null ${files} | DJANGO_SETTINGS_MODULE=spanner.test.settings ${command_wrapper} flake8 --diff
    exit
}

function run_sphinx {
    echo "Building sphinx..."
    export DJANGO_SETTINGS_MODULE=spanner.test.settings
...
function backup_environment {
  if [ $backup_env -eq 1 ]; then
    ...
    if [ -d /tmp/.spanner_environment/$JOB_NAME ]; then
      mv /tmp/.spanner_environment/$JOB_NAME /tmp/.spanner_environment/$JOB_NAME.old
      rm -rf /tmp/.spanner_environment/$JOB_NAME
    fi
    mkdir -p /tmp/.spanner_environment/$JOB_NAME
    cp -r $venv /tmp/.spanner_environment/$JOB_NAME/
    cp .environment_version /tmp/.spanner_environment/$JOB_NAME/
    # Remove the backup now that we've completed successfully
    rm -rf /tmp/.spanner_environment/$JOB_NAME.old
...
function restore_environment {
  if [ $restore_env -eq 1 ]; then
    ...
    cp -r /tmp/.spanner_environment/$JOB_NAME/.venv ./ || true
...
function run_tests_all {
  echo "Running Spanner application tests"
  export NOSE_XUNIT_FILE=spanner/nosetests.xml
  if [ "$NOSE_WITH_HTML_OUTPUT" = '1' ]; then
    export NOSE_HTML_OUT_FILE='spanner_nose_results.html'
  fi
  if [ $with_coverage -eq 1 ]; then
    ${command_wrapper} python -m coverage.__main__ erase
    coverage_run="python -m coverage.__main__ run -p"
  fi
  ${command_wrapper} ${coverage_run} $root/manage.py test spanner --settings=spanner.test.settings $testopts
  # get results of the Horizon tests
  SPANNER_DASHBOARD_RESULT=$?

  if [ $with_coverage -eq 1 ]; then
    echo "Generating coverage reports"
    ${command_wrapper} python -m coverage.__main__ combine
    ${command_wrapper} python -m coverage.__main__ xml -i --include="spanner/*" --omit='/usr*,setup.py,*egg*,.venv/*'
    ${command_wrapper} python -m coverage.__main__ html -i --include="spanner/*" --omit='/usr*,setup.py,*egg*,.venv/*' -d reports
  fi
  ...
  TEST_RESULT=$(($SPANNER_DASHBOARD_RESULT || $PEP8_RESULT))
